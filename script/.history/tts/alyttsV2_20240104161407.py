...
class AlyTTS:
    def __init__(self, speaker, long_text_speaker):
        self.speaker = speaker
        self.long_text_speaker = long_text_speaker
        self.appkey = configer.program_param("AL_APP_KEY")
        self.token_expireTime = int(configer.program_param("AL_TOKEN_EXPIRETIME"))
        self.access_token = configer.program_param("AL_ACCESS_TOCKEN")
        self.url = configer.program_param("AL_URL")  # 短文本合成地址

        self.special_word = [["Li Mei", "LeeMei"], ["Andy's", "Andy is"], ["°C", " degree celsius"], ["Hmm", "en"],
                             ["119", "One One Nine"],
                             ["110", "One One Zero"], ["120", "One Two Zero"], ["122", "One Two Two"],
                             ["911", "Nine One One"], ["Chang'e", "Chang er"], ["Shh", "she"], ["metres", "miters"],
                             ["lives", "leafs"], ["who's", "whose"], ["Who's", "whose"],

                             # 专有名词直接替换成中文发音
                             ["jiaozi", "饺子"], ["Bingmayong", "兵马俑"], ["taijiquan", "太极拳"], ["erhu", "二胡"],
    ...
    def waitLoop4Complete(self, url, appkey, token, task_id, request_id):
        ...
        while True:
            ...
            if ("data" in jsonData) and (
                    "audio_address" in jsonData["data"] and jsonData["data"]["audio_address"] != None):
    ...
    def replace_operational_sound(self, line):
        symbol_list = [["+", " plus ", ], ["-", "minus"], ["×", " times ", ], ["÷", " divided by ", ],
                       [".", " point ", ], ]
...
class TtsRequest:
    ...
    def tojson(self, e):
        return {'voice': e.voice, 'sample_rate': e.sample_rate, 'format': e.format,
                'enable_subtitle': e.enable_subtitle, 'text': e.text, 'speech_rate': e.speech_rate}
